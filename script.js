...
// PYQ subjects data
const pyq_subjects = [
    { name: "Bangla 1st Paper", link: "#" },
    { name: "Bangla 2nd Paper", link: "#" },
    { name: "English 1st & 2nd Paper", link: "#" },
    { name: "Biology", link: "#" },
    { name: "Chemistry", link: "#" },
    { name: "Physics", link: "#" },
    { name: "Mathematics", link: "#" },
    { name: "Higher Mathematics", link: "#" },
    { name: "BGS", link: "#" },
    { name: "Islam & M.E.", link: "#" },
    { name: "ICT", link: "#" }
];
...
// Open PYQ subject page function
function openPyqSubjectPage() {
    const pyqSubjectList = document.getElementById('pyq-subject-list');
    pyqSubjectList.innerHTML = '';

    pyq_subjects.forEach(subject => {
        const subjectItem = document.createElement('div');
        subjectItem.className = 'chapter-item';
        subjectItem.innerHTML = `
            <div class="chapter-name">${subject.name}</div>
            <div class="chapter-icon">&gt;</div>
        `;
        subjectItem.addEventListener('click', () => {
            // You can set the links for each subject in the `pyq_subjects` array
            if(subject.link !== "#") {
                window.open(subject.link, '_blank');
            }
        });
        pyqSubjectList.appendChild(subjectItem);
    });

    // Switch to PYQ subject page
    document.getElementById('home-page').classList.remove('active');
    document.getElementById('pyq-subject-page').classList.add('active');
...
// Handle back button from PYQ page
function goBackFromPyq() {
    document.getElementById('pyq-subject-page').classList.remove('active');
    document.getElementById('home-page').classList.add('active');
    window.scrollTo(0, 0);
}
